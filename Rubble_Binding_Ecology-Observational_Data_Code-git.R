# Kenyon et al. (2024) Ecosphere - Observational Data Code
...
dat %>% filter(!is.na(bound)) %>% 
  ...
  geom_bar(aes(fill=bound), position = "stack") +
...
# Morphology across depth and exposure

datsum <- dat %>% group_by(aspectdepth, morphology_simple) %>%
  summarise(numberpiece = n())

# Proportion of rubble of each morphology:

datsum2 <- datsum %>% group_by(aspectdepth) %>% mutate(total = sum(numberpiece)) %>%
  summarise(prop = numberpiece/total) %>% ungroup()

# Reef flat: 99% were cylindrical pieces from branching/digitate corals
# BUT 65% were unbranched, whereas the proportion of unbranched on the reef slope was 
# West 2-3m: 92% - 34% unbranched
# West 6-7m: 87% - 31% unbranched
# SE 2-3m: 89% - 22%
# SE 6-7m: 92%  - 35%

datsum3 <- inner_join(datsum, datsum2)

write.csv(datsum3, "datsum3.csv") # This is Table S48 in Supp.


# Plot this:

ggplot(datsum, aes(x = morphology_simple, y = numberpiece, colour = morphology_simple)) +
  geom_point() + facet_wrap(~aspectdepth)
