# Kenyon et al. 2024 Ecology submission
...
mo3_18 <- lm(total_binds_per_piece ~ aspect_depth * month * side,
                data=d1_3_18)

mo3_182 <- lm(log(total_binds_per_piece+1) ~ aspect_depth * month * side, data=d1_3_18)

plot(simulateResiduals(fittedModel = mo3_18)) # looks good
plot(simulateResiduals(fittedModel = mo3_182)) # looks good

car::Anova(mo3_182) # sig. effect of aspectdepth:month, remove non-significant terms

mo3_182_2 <- update(mo3_182, ~ . - aspect_depth:month:side)

car::Anova(mo3_182_2)

mo3_182_3 <- update(mo3_182_2, ~ . - aspect_depth:side)

car::Anova(mo3_182_3)

mo3_182_4 <- update(mo3_182_3, ~ . - month:side)

car::Anova(mo3_182_4)

r.squaredGLMM(mo3_182_4) # R squared is 57%
plot(simulateResiduals(fittedModel = mo3_182_4)) # looks good

write.csv(car::Anova(mo3_182_4), file = "car::Anova(mo3_182_4).csv")

emmeans(mo3_182_4, pairwise ~ aspect_depth | month, type = "response")

pred3_18 <- emmeans(mo3_182_4, pairwise ~ month | aspect_depth, type = "response")
...
pred3_18M <- emmeans(mo3_182_4, pairwise ~ aspect_depth | month, type = "response")
...
hist(d1total$total_binds_per_piece_both_side+1)

mo_d1total <- lm(log(total_binds_per_piece_both_side+1) ~ aspect_depth * month, data=d1total)
...
# topside only can be considered here.

d12.5 <- filter(d1, month == 2.5) %>% droplevels()

hist(d12.5$total_binds_per_piece) 
hist(log(d12.5$total_binds_per_piece+1)) 

mo2.5comb <- lm(total_binds_per_piece ~ aspect_depth,
                data=d12.5)

mo2.5comb2 <- lm(log(total_binds_per_piece+1) ~ aspect_depth,
                 data=d12.5)

plot(simulateResiduals(fittedModel = mo2.5comb)) # quantile deviations
plot(simulateResiduals(fittedModel = mo2.5comb2)) # good

car::Anova(mo2.5comb2) # sig. effect of aspectdepth

r.squaredGLMM(mo2.5comb2) # R squared is 41%

write.csv(car::Anova(mo2.5comb2), file = "car::Anova(mo2.5comb2).csv")

# Pairwise comps between aspect_depth

emmeans(mo2.5comb2, pairwise ~ aspect_depth, type = "response")

pred2 <- emmeans(mo2.5comb2, pairwise ~ aspect_depth, type = "response")

pred2.1 <- pred2$emmeans %>% as.data.frame()
pred2.1.t2 <- flextable(pred2.1) %>% 
  colformat_double(big.mark = ",", digits = 2, na_str = "N/A") %>%
  save_as_docx(path = "pred2.1.t2.docx")

pred2.2 <- pred2$contrasts %>% as.data.frame()
pred2.2.t <- flextable(pred2.2) %>% 
  colformat_double(big.mark = ",", digits = 2, na_str = "N/A") %>%
  save_as_docx(path = "pred2.2.t.docx")
...
mo3_18T <- lm(total_binds_per_piece ~ aspect_depth * month,
             data=d1_3_18T2)

mo3_18T2 <- lm(log(total_binds_per_piece+1) ~ aspect_depth * month, data=d1_3_18T2)

plot(simulateResiduals(fittedModel = mo3_18T)) # looks good
plot(simulateResiduals(fittedModel = mo3_18T2)) # looks better

car::Anova(mo3_18T2) # sig. effect of aspectdepth:month

r.squaredGLMM(mo3_18T2) # R squared is 76%
...
# Conducted in R due to lack of random effects (random effect of site should have been included,
# but there was a low level of replication - only 2 'counts' per site, so it could not be included.)
...
# as the binds were spread pretty evenly across the grid.
